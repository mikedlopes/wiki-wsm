...
| **Operations & projects** | Active jobs **and** preconstruction: project folders (`/<ProjectID>-<ClientShort>/`), **estimating & bids** (bid files, addenda, bid/no-bid), submittals/closeout, field/shop turnover | PM, field, shop, design, estimating, BD | [Projects](/projects/dashboard), [Project home template](/projects/template-project/project-home), [Estimating](/departments/estimating/overview), [Bidding](/departments/bidding/overview) | **PRJ-Active** / **PRJ-Archive**; **Bids** library + optional **Bids** list; same metadata model | **No separate Estimating site** — bids live here to reduce sprawl. Optional **Standards** / **Templates** libraries on this site or hub. |
...
### Current rollout (minimal sites)

**Ship first:** **Organization hub** + **Operations & projects** (includes estimating/bids content in libraries or libraries under that site). **Accounting** is optional for v1—document a **placeholder** URL or “TBD” until restricted libraries and owners are ready.
...
| `/departments/estimating/*` | **Operations** site — **Bids** library/area + lists (no separate Estimating site) | Preconstruction files alongside project delivery in one tenant footprint. |
| `/departments/bidding/*` | Same **Operations** site — **Bids** library/list | Distinct library or metadata from active job folders. |
...
**Convention:** Live **wiki** folders use `projects/<ProjectID>/` (e.g. `TI-2026-045`). New jobs are cloned from `projects/template-project/` — not from a literal `[project-id]` folder name. **Project documents** in SharePoint (or on a file share) use a parallel root `/<ProjectID>-<ClientShort>/` with **numbered** folders `00_`–`07_` (submittals, RFIs, permits, testing/inspection, calcs, etc.), **alphabetical** commercial folders (Bid … Template Forms), **Invoices**, **Material Listing**, **Purchase Orders**, and optional `_QuickLinks/` — see [Project home template](/projects/template-project/project-home).
